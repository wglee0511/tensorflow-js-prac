...
import { getBostonHousingPriceModel, getBostonHousingPriceModelCallbacks } from '@/model/BostonHousingPrice';
...
export const bostonHousingTrain = async () => {
  ...
  let trainLogs: { loss: number; val_loss: number }[] = [];

  await bostonHousingDataSet.loadData();

  const bostonHousingModel = getBostonHousingPriceModel({ numFeatures: bostonHousingDataSet.numFeatures });
  ...
  await bostonHousingModel.fit(trainFeatures as Tensor, trainTarget as Tensor, {
    ...
    validationSplit: 0.2,
    callbacks: {
      onEpochEnd: async (epoch, logs) => onEpochEnd({ epoch, logs, numEpochs: NUM_EPOCHS, trainLogs: trainLogs }),
    },
  });
