...
export type DataByClassIris = number[][][];
export type TargetByClassIris = number[][];
...
export const getIrisData = (
  ...
  const tensorflowTidy = tf.tidy(() => {
    ...
    const xTrains: Tensor[] = [];
    const yTrains: Tensor[] = [];
    const xTests: Tensor[] = [];
    const yTests: Tensor[] = [];

    forEach(IRIS_CLASSES, () => {
      dataByClass.push([]);
      targetByClass.push([]);
    });
    ...
    forEach(IRIS_CLASSES, (_item, index) => {
      const { xTrain, xTest, yTrain, yTest } = getIrisTensors({
        dataByClass: dataByClass[index],
        targetByClass: targetByClass[index],
        testSplit,
      });
      xTrains.push(xTrain);
      yTrains.push(yTrain);
    ...
    return {
      tfXTrains: tf.concat(xTrains, concatAxis),
      tfYTrains: tf.concat(yTrains, concatAxis),
      tfXTests: tf.concat(xTests, concatAxis),
      tfYTests: tf.concat(yTests, concatAxis),
    };
  });

  return tensorflowTidy;
};
